...
export default function MockQrDetailPage({
  params,
}: {
...
function QrDetail({ id }: { id: string }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const { qr_code } = await mockQrApi.get(id);
        if (cancelled) return;
        setQr(qr_code);
        if (qr_code.entity_id) {
          const [entRes, privRes] = await Promise.allSettled([
            mockEntitiesApi.get(qr_code.entity_id),
            mockEntitiesApi.getPrivacy(qr_code.entity_id),
          ]);
          if (cancelled) return;
  ...
  const saveVehicle = async (e: FormEvent) => {
    ...
    setSaveError(null);
    try {
      const { entity: updated } = await mockEntitiesApi.update(entity.id, {
  ...
  const togglePrivacy = async (key: PrivacyKey) => {
    ...
    try {
      const { privacy: updated } = await mockEntitiesApi.updatePrivacy(
        entity.id,
        { [key]: next }
      );
      setPrivacy(updated);
