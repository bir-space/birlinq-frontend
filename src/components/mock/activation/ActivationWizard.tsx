...
export function ActivationWizard() {
  ...
  const activateHref = code
    ? `/mock/activate?code=${encodeURIComponent(code)}&token=${encodeURIComponent(token)}`
    : "/mock/activate";
  const loginNextHref = `/mock/login?next=${encodeURIComponent(activateHref)}`;
